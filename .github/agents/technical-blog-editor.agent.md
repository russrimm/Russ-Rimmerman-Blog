...
- `heroImage` and `heroAlt` (optional): if a hero image is referenced, ensure `heroAlt` is meaningful, not decorative filler. When generating a hero image prompt for a post, follow the Hero Image Prompt Rules in `.github/skills/blog-post/SKILL.md` — every prompt must vary pose, expression, framing, and wardrobe color from prior posts, dress the subject in business casual (khakis and a short-sleeved collared polo shirt), and feature a realistic markerboard with hand-drawn labels and applicable brand/product logos tied to that specific post's content.
