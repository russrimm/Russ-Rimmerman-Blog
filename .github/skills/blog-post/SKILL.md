...
9. **Ensure hero image inputs are accurate.** Hero images are generated by `scripts/generate-hero.mjs` (`npm run hero`), which owns the full image prompt. Do not hand-author a separate hero image prompt. Instead, make sure the post's `title`, `description`, and `tags` are specific and accurate — the script uses them to decide what the completed hand-drawn diagram on the whiteboard depicts and which product labels and sketched logos appear.

10. **Vibe coding posts assume a non-coder reader (required).** For any post about vibe coding, agentic coding, GitHub Copilot, Copilot CLI, or the GitHub Copilot app, write for someone whose first exposure to code is this article. Define every developer term at first use, either with a hover tooltip (`import Term from "@/components/Term.astro";` then `<Term def="Plain-language definition.">the word</Term>`) or with an inline explanation in the prose. Use `Explain` blocks for concepts that need a paragraph. Never let a definition contain other undefined jargon, and never simplify commands, paths, or product names. See "Vibe coding posts: assume the reader has never written code" in `.github/agents/technical-blog-editor.agent.md`.

11. **Run the Humanizer pass (required).** Before the post is done, run the `humanizer` skill (`.github/skills/humanizer/SKILL.md`) over the full draft. Strip AI-tell openers, empty transitions, marketing hype, and formulaic closes, and rewrite anything that sounds robotic into a warm, specific, human voice. A generated post is not finished until it passes the Humanizer definition of done.

## Hero Image Prompt Rules

**Source of truth: `scripts/generate-hero.mjs`.**

Hero images for this blog are generated by the `npm run hero` script (which also runs in the `generate-hero-images.yml` workflow). That script owns the full prompt: scene, wardrobe, markerboard rules, logo policy, spelling policy, everything. Do not hand-author a separate hero image prompt for a post and do not paste alternate hero rules into other skills or agents.

When writing a post, the only hero-image-related things to make sure are true are:

- The post has good `tags` in its frontmatter. The script uses the tag list to decide which products/services get labels and sketched logos on the whiteboard, so the tags should include every technology the diagram should reference (for example: `Copilot Studio`, `Power Platform`, `Azure`, `Dataverse`, `GitHub`, `VS Code`, `Teams`, `SharePoint`, `Dynamics 365`, `Foundry`, `Intune`, `Entra`).
- The `title` and `description` are clear and specific. The script feeds them into the prompt to decide what the completed hand-drawn diagram on the whiteboard actually depicts.

If you need to change how hero images look (composition, wardrobe, allowed logos, spelling policy, avoid-list, and so on), edit `scripts/generate-hero.mjs` and regenerate. Never fork the prompt into this file.
